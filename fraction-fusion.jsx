...
var DIFFICULTIES = {
  ...
  normal: { label: "\u3075\u3064\u3046", emoji: "\u26a1", cardCount: 15, ds: [2, 3, 4, 5, 6, 8, 10, 12], splits: 3, color: "#FFEAA7", desc: "15\u679a\u30fb\u5206\u6bcd12\u307e\u3067", bonus: 1.0 },
...
function FormulaOverlay(props) {
  ...
  useEffect(function() {
    var t1, t2;
    if (needsConvert) { t1 = setTimeout(function() { setPhase(1); }, 800); t2 = setTimeout(function() { setPhase(2); }, 1800); }
    else { t2 = setTimeout(function() { setPhase(2); }, 1000); }
    return function() { clearTimeout(t1); clearTimeout(t2); };
  }, [needsConvert]);
  useEffect(function() {
    if (phase === 2) { var t = setTimeout(function() { onConfirm(result); }, 1200); return function() { clearTimeout(t); }; }
  }, [phase]);
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 800, background: "rgba(0,0,0,.5)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "linear-gradient(145deg,#1a1a2e,#16213e)", borderRadius: 24, padding: "24px 20px", boxShadow: "0 20px 60px rgba(0,0,0,.6)", border: "2px solid rgba(255,255,255,.1)", animation: "formulaIn .4s ease-out", maxWidth: "92vw", width: 340, textAlign: "center" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginBottom: needsConvert ? 12 : 16, padding: "8px 0" }}>
          ...
          {phase < 2 ? <span style={{ fontSize: 28, fontWeight: 700, color: "rgba(255,255,255,.2)", fontFamily: FF }}>?</span> : <span style={{ animation: "answerReveal .4s ease-out" }}><Frac n={result.n} d={result.d} size={24} color="#FFD700" /></span>}
        </div>
        {needsConvert && (
          <div style={{ opacity: phase >= 1 ? 1 : 0.15, transition: "opacity .5s", marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "rgba(116,185,255,.06)", borderRadius: 14, padding: "8px 12px" }}>
              ...
              {phase < 2 ? <span style={{ fontSize: 22, color: "rgba(255,255,255,.15)", fontFamily: FF }}>?</span> : <span style={{ animation: "answerReveal .4s ease-out" }}><Frac n={result.n} d={result.d} size={20} color="#FFD700" bold={false} /></span>}
            </div>
          </div>
        )}
        {phase < 2 && <div style={{ fontSize: 11, color: "rgba(255,255,255,.2)", marginTop: 4 }}>{"\u8a08\u7b97\u4e2d..."}</div>}
...
function ResultScr(props) {
  var del = props.del, cards = props.cards, onRetry = props.onRetry, diff = props.diff;
  var sc = calcSc(del, cards, diff);
  var rank = getRank(sc.total);
  var cfg = DIFFICULTIES[diff] || DIFFICULTIES.normal;
  var maxScore = 6000;
  var pct = Math.min(100, Math.round(sc.total / maxScore * 100));
  var rnk = pct >= 90 ? "S" : pct >= 75 ? "A" : pct >= 55 ? "B" : pct >= 30 ? "C" : "D";

  /* TRAIL連携: 結果をparentに送信（iframe内の場合のみ動作） */
  useEffect(function() {
    try {
      if (window.parent && window.parent !== window) {
        window.parent.postMessage({
          type: "trail:game:result",
          payload: {
            score: sc.total,
            max_score: maxScore,
            percent: pct,
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 900, background: "rgba(0,0,0,.6)", backdropFilter: "blur(10px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 12 }}>
      <div style={{ background: "linear-gradient(145deg,#1a1a2e,#16213e)", borderRadius: 22, padding: "24px 20px", textAlign: "center", boxShadow: "0 30px 80px rgba(0,0,0,.6)", border: sc.perf ? "2px solid rgba(255,215,0,.3)" : "2px solid rgba(255,255,255,.08)", animation: "fadeIn .5s ease-out", maxWidth: 380, width: "100%" }}>
        <div style={{ fontSize: 40, marginBottom: 2 }}>{sc.perf ? "\ud83c\udfc6" : rank.e}</div>
        <div style={{ fontSize: 20, fontWeight: 700, fontFamily: FF, color: sc.perf ? "#FFD700" : rank.c, marginBottom: 8 }}>{sc.perf ? "PERFECT!" : rank.l}</div>
        <div style={{ fontSize: 32, fontWeight: 700, fontFamily: FF, background: "linear-gradient(135deg,#FFD700,#FFA500)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", marginBottom: 16 }}>{sc.total}</div>
        {del.length > 0 && (
          <div style={{ display: "flex", gap: 5, justifyContent: "center", marginBottom: 12, flexWrap: "wrap" }}>
            {del.map(function(d, i) {
              return (
                <div key={i} style={{ width: 36, height: 36, borderRadius: 9, background: "linear-gradient(135deg,#FFD700,#FF8C00)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 3px 10px rgba(255,215,0,.3)" }}>
                  <span style={{ fontSize: 15, fontWeight: 700, color: "#1a1a2e", fontFamily: FF }}>1</span>
                </div>
              );
            })}
          </div>
        )}
        <button onClick={onRetry} style={{ background: "linear-gradient(135deg,#667eea,#764ba2)", border: "none", borderRadius: 14, padding: "11px 30px", color: "#fff", fontSize: 15, fontWeight: 700, fontFamily: FF, cursor: "pointer" }}>{"\u3082\u3046\u4e00\u5ea6\u6311\u6226 \u21bb"}</button>
      </div>
    </div>
  );
}
...
/* ======== MAIN ======== */
export default function FractionFusion() {
  ...
  var onSpChoose = useCallback(function(newFrac) {
    if (!spOp) return;
    saveH();
    setCards(function(p) { return p.map(function(c) { return c.id === spOp.target.id ? Object.assign({}, c, { n: newFrac.n, d: newFrac.d }) : c; }); });
  ...
  var finish = useCallback(function() { setResult(true); setCMenu(null); }, []);
  var startGame = useCallback(function(d) {
    setDiff(d); setCards(makePuzzle(d)); setRules(false); setResult(false);
  ...
  var retry = useCallback(function() {
    ...
    setFormula(null); setSpUses(3);
  }, [diff]);
  var backToMenu = useCallback(function() {
    setDiff(null); setRules(true); setResult(false); setCards([]); setDel([]);
  ...
  /* TRAIL連携: URLパラメータで自動開始 */
  useEffect(function() {
    try {
      var params = new URLSearchParams(window.location.search);
      var d = params.get("difficulty");
      if (d && DIFFICULTIES[d]) { startGame(d); }
    } catch (e) { /* ignore */ }
  }, []);
  ...
  if (rules) {
    return (
      <div style={{ width: "100%", minHeight: "100vh", background: "linear-gradient(150deg,#0a0a1a 0%,#141432 40%,#0d1b2a 100%)", fontFamily: FF, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20, userSelect: "none" }}>
        <style>{CSS}</style>
        <div style={{ animation: "fadeIn .5s ease-out", textAlign: "center", maxWidth: 380, width: "100%" }}>
          <div style={{ fontSize: 48, marginBottom: 8 }}>{"\ud83e\uddea"}</div>
          <div style={{ fontSize: 26, fontWeight: 700, fontFamily: FF, background: "linear-gradient(135deg,#FF6B6B,#FFEAA7,#4ECDC4)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", marginBottom: 6 }}>{"\u5206\u6570\u878d\u5408"}</div>
          <div style={{ fontSize: 13, color: "rgba(255,255,255,.4)", marginBottom: 24 }}>{"\u30ab\u30fc\u30c9\u3092\u305f\u3057\u7b97\u3067\u5408\u4f53\u3055\u305b\u3066\u300c1\u300d\u3092\u4f5c\u308d\u3046\uff01"}</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 24 }}>
            {["easy", "normal", "hard"].map(function(key) {
              var d = DIFFICULTIES[key];
              return (
                <button key={key} onClick={function() { startGame(key); }} style={{ background: "rgba(255,255,255,.04)", border: "2px solid " + d.color + "44", borderRadius: 16, padding: "16px 20px", cursor: "pointer", display: "flex", alignItems: "center", gap: 14, transition: "all .2s" }}>
                  <div style={{ fontSize: 32 }}>{d.emoji}</div>
                  <div style={{ textAlign: "left", flex: 1 }}>
                    <div style={{ fontSize: 18, fontWeight: 700, color: d.color, fontFamily: FF }}>{d.label}</div>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,.4)", fontFamily: FF }}>{d.desc}</div>
                  </div>
                </button>
              );
            })}
          </div>
          <div style={{ background: "rgba(255,255,255,.03)", borderRadius: 14, padding: "14px 16px", textAlign: "left" }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,.5)", marginBottom: 8 }}>{"\ud83c\udfae \u3042\u305d\u3073\u304b\u305f"}</div>
            {["\ud83d\udc46 \u30ab\u30fc\u30c9\u3092\u91cd\u306d\u308b \u2192 \u305f\u3057\u7b97\u3067\u5408\u4f53", "\ud83d\udc47 \u30bf\u30c3\u30d7 \u2192 \u7d04\u5206\u30fb\u901a\u5206", "\ud83d\udce6 1\u3092\u4f5c\u3063\u305f\u3089\u7d0d\u54c1\u30dc\u30bf\u30f3\u3067\u7d0d\u54c1", "\u2b50 \u30b9\u30da\u30b7\u30e3\u30eb(2,3,5,7)\u3092\u30ab\u30fc\u30c9\u306b\u91cd\u306d\u3066\u00d7\u00f7\uff083\u56de\uff09"].map(function(t, i) {
  ...
  /* ======== GAME SCREEN ======== */
  return (
    <div style={{ width: "100%", height: "100dvh", background: "linear-gradient(150deg,#0a0a1a 0%,#141432 40%,#0d1b2a 100%)", fontFamily: FF, position: "relative", overflow: "hidden", userSelect: "none", touchAction: "none" }}>
      <style>{CSS}</style>

      {/* === HEADER: 0-30px === */}
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 30, padding: "0 8px", display: "flex", alignItems: "center", gap: 4, zIndex: 30, background: "#0a0a1a" }}>
        <button onClick={backToMenu} style={{ background: "none", border: "none", fontSize: 14, color: "rgba(255,255,255,.4)", cursor: "pointer", padding: "2px" }}>{"\u2190"}</button>
        <div style={{ fontSize: 13, fontWeight: 700, background: "linear-gradient(135deg,#FF6B6B,#FFEAA7,#4ECDC4)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{"\u5206\u6570\u878d\u5408"}</div>
        <span style={{ fontSize: 8, color: cfg.color, background: cfg.color + "22", borderRadius: 5, padding: "1px 4px", fontWeight: 600 }}>{cfg.emoji + cfg.label}</span>
        <div style={{ flex: 1 }} />
        <button onClick={undo} disabled={hist.length === 0} style={{ background: hist.length ? "rgba(255,255,255,.08)" : "transparent", border: "none", borderRadius: 6, padding: "3px 6px", color: hist.length ? "rgba(255,255,255,.5)" : "rgba(255,255,255,.1)", fontSize: 10, fontFamily: FF, cursor: "pointer" }}>{"\u21a9"}</button>
        <button onClick={retry} style={{ background: "rgba(255,107,107,.1)", border: "none", borderRadius: 6, padding: "3px 6px", color: "#FF6B6B", fontSize: 10, fontFamily: FF, cursor: "pointer" }}>{"\u21bb"}</button>
        <span style={{ fontSize: 9, color: "rgba(255,255,255,.2)" }}>{"\u6b8b" + cards.length}</span>
        <div style={{ background: "linear-gradient(135deg,#667eea,#764ba2)", borderRadius: 6, padding: "1px 8px", fontSize: 11, fontWeight: 700, color: "#fff" }}>{live.total}</div>
      </div>

      {/* === CARDS: 30px to 70% === */}
      <div style={{ position: "absolute", top: 30, left: 6, right: 6, bottom: "30dvh", overflow: "hidden", zIndex: 10 }}>
        <div style={{ width: "100%", height: "100%", display: "grid", gridTemplateColumns: "repeat(" + gridCols + ",1fr)", gridTemplateRows: "repeat(" + gridRows + ",1fr)", gap: 4, touchAction: "none" }}>
          {cards.map(function(card) {
            ...
            var bdr = isHv ? "3px solid #74B9FF" : c1 ? "2px solid rgba(255,215,0,.5)" : "1px solid rgba(255,255,255,.08)";
            return (
              <div key={card.id} data-cid={card.id} onPointerDown={function(e) { onPD(e, card); }}
                style={{ borderRadius: 10, gridColumn: isLast ? "1/-1" : undefined, maxWidth: isLast ? 100 : undefined, justifySelf: isLast ? "center" : undefined, background: bg, border: bdr, cursor: isDg ? "grabbing" : "grab", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", zIndex: isDg ? 100 : 1, transform: isDg ? "translate(" + dx + "px," + dy + "px) scale(1.15) rotate(-3deg)" : isHv ? "scale(1.08)" : "scale(1)", transition: isDg ? "none" : "all .2s", animation: isHv ? "dropTarget .6s infinite" : c1 ? "winGlow 2s infinite" : isDg ? "none" : "cardAppear .35s ease-out", boxShadow: isDg ? "0 20px 50px rgba(0,0,0,.6)" : c1 ? "0 6px 20px rgba(255,215,0,.3)" : "0 2px 8px rgba(0,0,0,.3)", minHeight: 0, minWidth: 0 }}>
                ...
                {cu1 && <div style={{ position: "absolute", top: 1, right: 1, fontSize: 6, color: "#fff", fontWeight: 700, background: "rgba(0,0,0,.5)", borderRadius: 3, padding: "0 2px" }}>{"\u7d04\u21921"}</div>}
                {card.d === 1 ? (
      ...
      {/* === BOTTOM: 30dvh === */}
      <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, height: "30dvh", display: "flex", gap: 6, padding: "6px", alignItems: "stretch", zIndex: 20, background: "#0d0d20", borderTop: "2px solid rgba(255,255,255,.08)" }}>
        {/* SP cards - flex:1 = half width */}
            ...
            {SP_NUMS.map(function(num) {
              ...
              return (
                <div key={num} onPointerDown={function(e) { onSpDown(e, num); }}
                  ...
                  <span style={{ fontSize: 24, fontWeight: 700, color: "#fff", fontFamily: FF }}>{num}</span>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 4, minWidth: 0 }}>
          ...
          <div id="dock" style={{ flex: 1, borderRadius: 16, padding: "8px 10px", background: "linear-gradient(135deg, rgba(255,215,0,.1), rgba(255,165,0,.06))", border: "2.5px dashed rgba(255,215,0,.45)", animation: "dockGlow 2.5s infinite", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", gap: 6, overflow: "hidden", minHeight: 0 }}>
            ...
            {!hasOnes && del.length === 0 && <span style={{ fontSize: 11, color: "rgba(255,215,0,.4)", fontFamily: FF, textAlign: "center" }}>{"\u300c1\u300d\u3092\u4f5c\u3063\u3066\u7d0d\u54c1"}</span>}
            {del.length > 0 && (
                ...
                {del.map(function(d, i) {
                  return (
                    <div key={i} style={{ width: 22, height: 22, borderRadius: 5, background: "linear-gradient(135deg,#FFD700,#FF8C00)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 1px 4px rgba(255,215,0,.3)", flexShrink: 0 }}>
                      <span style={{ fontSize: 10, fontWeight: 700, color: "#1a1a2e", fontFamily: FF }}>1</span>
                    </div>
                  );
                })}
              </div>
          ...
          {/* Finish button */}
          <button onClick={finish} style={{ borderRadius: 14, border: "none", padding: "10px 0", background: finishActive ? "linear-gradient(135deg,#667eea,#764ba2)" : "rgba(255,255,255,.05)", color: finishActive ? "#fff" : "rgba(255,255,255,.25)", fontFamily: FF, fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, flexShrink: 0, boxShadow: finishActive ? "0 3px 12px rgba(102,126,234,.4)" : "none" }}>
            <span style={{ fontSize: 20 }}>{cards.length === 1 ? "\ud83c\udfc6" : "\u2728"}</span>
            <span>{del.length > 0 || cards.length === 1 ? "\u7d50\u679c\u3092\u898b\u308b" : "\u7d42\u4e86"}</span>
          </button>
        </div>
      </div>
      ...
      {result && <ResultScr del={del} cards={cards} onRetry={retry} diff={diff} />}
    </div>
  );
}
